  ...
  return (
    <div className="grid grid-cols-1 gap-4 md:grid-cols-[200px_1fr]">
      <aside className="flex flex-col gap-1 rounded-xl border border-black/10 bg-white p-3 dark:border-white/10 dark:bg-neutral-900 md:h-[70vh]">
        <Link
          href={`/communities/${slug}/chat`}
          className="mb-1 rounded-lg px-3 py-2 text-xs font-medium text-neutral-400 hover:bg-black/5 dark:hover:bg-white/10"
        >
          ← Todas las salas
        </Link>
        {rooms.map((r) => {
          ...
          return (
            <Link
              key={r.id}
              href={isMember ? `/communities/${slug}/chat/${r.id}` : `/communities/${slug}/chat`}
              className={
                r.id === roomId
                  ...
                  : isMember
                    ...
                    : "rounded-lg px-3 py-2 text-sm text-neutral-400 hover:bg-black/5 dark:hover:bg-white/10"
              }
            >
              # {r.name}
            </Link>
